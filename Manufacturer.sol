...
contract Manufacturer is Stakeholder {
  ...
  function addRawProduct(string memory _name, address[] memory _supplierIds, bool[] memory _isVerified) public returns (bool){
    ...
    require(_supplierIds.length == _isVerified.length, "Manufacturer::addRawProduct: Length of supplierIds and isVerified arrays should match");
    for(uint i =0; i < _supplierIds.length; i++){
      _rawProductSuppliers[msg.sender][_name].push(_supplierIds[i]);
      _rawProductVerified[msg.sender][_name] = _rawProductVerified[msg.sender][_name] && _isVerified[i];
    }
    return true;
}
